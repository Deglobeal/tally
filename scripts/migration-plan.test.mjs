...
test("the auth source schema ships outside the globbed directory", () => {
  const migrationsDir = join(projectRoot(), "migrations");

  // The appliers only consume the root migration directory. The auth source
  // remains in migrations/auth/ and is not itself discovered by readdir().
  assert.ok(readdirSync(migrationsDir).includes("auth"));
  assert.ok(readdirSync(join(migrationsDir, "auth")).includes(AUTH_MIGRATION));

  const discovered = pendingMigrations(readdirSync(migrationsDir), []);
  assert.deepEqual(
    discovered.map(({ name }) => name),
    ["0001_auth.sql", "0002_tasks.sql", "0003_reminders.sql"],
  );
});
