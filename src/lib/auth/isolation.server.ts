 ...
 *
 * Same-site deployments can still be exposed to sibling-origin requests, and a
 * `SameSite=Lax` session cookie can be sent on same-site subrequests — so
 * without this, a malicious sibling could make a SCRIPTED
...
export function assertSameSiteRequest(): void {
  ...
  // A top-level GET navigation (e.g. the OAuth callback redirect) is
